...
class FMCRouteConverter:
    ...
    def get_existing_objects(self) -> None:
        ...
        try:
            ...
            for obj in response.json().get('items', []):
                if 'value' in obj and 'mask' in obj:
                    key = f"{obj['value']}/{obj['mask']}"
                    self.network_objects[key] = obj
                    print(f"Found network object: {obj['name']} ({key})")
        except requests.exceptions.RequestException as e:
        ...
        try:
            ...
            for obj in response.json().get('items', []):
                if 'value' in obj:
                    self.host_objects[obj['value']] = obj
                    print(f"Found host object: {obj['name']} ({obj['value']})")
        except requests.exceptions.RequestException as e:
    ...
    def find_or_create_object(self, value: str, mask: str = None) -> Dict:
        ...
        cache_key = value if is_host else f"{value}/{mask}"
        
        # Look for existing object
        if cache_key in cache:
            print(f"Found existing object: {cache[cache_key]['name']} for {cache_key}")
            return cache[cache_key]
        
        print(f"Warning: No existing object found for {cache_key}")
        return None
...
def main():
    ...
    FMC_HOST = "fmc.example.com"  # Replace with actual FMC hostname/IP
    USERNAME = "admin"  # Replace with actual username
    PASSWORD = "password"  # Replace with actual password
    DEVICE_NAME = "FTD_DEVICE"  # Replace with FTD device name as shown in FMC
    ROUTES_FILE = "ASA-static-routes.txt"  # Replace with actual file path
